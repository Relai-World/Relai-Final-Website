    ...
    const fetchNearbyPlaces = async () => {
      try {
        ...
        if (latitude && longitude && latitude !== 0 && longitude !== 0) {
          apiUrl = `/api/property-nearby-places?lat=${latitude}&lng=${longitude}`;
        } 
        // Fall back to property name for geocoding
        else if (propertyName) {
          apiUrl = `/api/property-nearby-places?propertyName=${encodeURIComponent(propertyName)}&location=Hyderabad`;
        } 
